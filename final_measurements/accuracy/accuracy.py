...
def calculate_tp_fp_tn_fn_x(recommendations, test_set, target_company, company_index, x):
    ...
    for i in range(len(recommendations)): 
        # Ensure actual_company is a string, handle floats and NaN
        actual_company = str(test_set.iloc[i, company_index]).strip().lower() if pd.notna(test_set.iloc[i, company_index]) else ""

        # Ensure predicted_companies are strings, handle floats and NaN
        predicted_companies = [
            str(recommendations.iloc[i, col]).strip().lower() if pd.notna(recommendations.iloc[i, col]) else ""
